...
const POST_REALTIME_TOPICS: PostRealtimeEventType[] = [
  "likes",
  "new-comment",
  "update-comment",
  "delete-comment",
  ...
  const subscriptions = POST_REALTIME_TOPICS.map((type) =>
    client.subscribe(`/topic/post/${postId}/${type}`, (message) => {
      postRealtimeListeners.get(postId)?.forEach((listener) => {
        try {
          listener({ type, body: message.body });
        } catch (error) {
          console.error(`[Post realtime] Invalid ${type} payload`, error);
        }
      });
    }),
  );
...
function ensurePostRealtimeClient() {
  if (postRealtimeIdleTimer) {
    clearTimeout(postRealtimeIdleTimer);
    postRealtimeIdleTimer = null;
  }
  ...
  const client = new Client({
    ...
    onConnect: () => {
      ...
      postRealtimeListeners.forEach((_listeners, postId) => subscribePostTopics(postId));
    },
    onWebSocketClose: () => {
      if (postRealtimeClient === client) postRealtimeSubscriptions.clear();
    },
    onStompError: (frame) => {
      console.error("[Post realtime]", frame.headers.message || frame.body);
    },
  });
...
export function subscribePostRealtime(postId: string, listener: PostRealtimeListener) {
  ...
  return () => {
    const activeListeners = postRealtimeListeners.get(normalizedPostId);
    activeListeners?.delete(listener);
    if (activeListeners?.size) return;

    postRealtimeListeners.delete(normalizedPostId);
    postRealtimeSubscriptions.get(normalizedPostId)?.forEach((subscription) => {
      try {
        subscription.unsubscribe();
      } catch {
        // The broker may already have closed the subscription.
      }
    });
